...
class add_date_event{
    ...
    static function metabox(){
        ...
        if(!isset($metadata['date_event'])){
           $data = "";
        }else{
            $data = $metadata['date_event'][0];
        };
        ?>
        <input type="date" format="d/m/Y"  name="<?php echo __CLASS__ ?>[date_event]" value="<?php echo  $data; ?>" style="background-color: #fff; font-size: 1.7em; height: 1.7em; line-height: 100%; margin: 0; outline: 0; padding: 3px 8px; width: 100%;"  required/>
        <?php
    }
