...
class add_hour_event{
    ...
    static function metabox(){
        ...
        if(!isset($metadata['hour_event'])){
           $hour = "";
        }else{
            $hour = $metadata['hour_event'][0];
        };
        ?>
        <input type="time" pattern="[0-9]{2}:[0-9]{2}"  name="<?php echo __CLASS__ ?>[hour_event]" value="<?php echo  $hour; ?>" style="background-color: #fff; font-size: 1.7em; height: 1.7em; line-height: 100%; margin: 0; outline: 0; padding: 3px 8px; width: 100%;"  required/>
        <?php
    }
